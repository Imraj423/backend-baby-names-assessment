...
def extract_names(filename):
    ...
    for name in sorted(dict1.keys()):
        # appending each name in the key of dict1 and drawing said value
        names.append('{} {}'.format(name, dict1[name]))
    return names
...
def main(args):
    ...
    for file_name in file_list:
        print(file_name)
        # text = '{}\n'.format('\n'.join(extract_names(thing)))
        # if create_summary:
        #     with open(thing + ".summary", 'w') as f:
        #         f.write(text)
        # else:
        #     print(text)
